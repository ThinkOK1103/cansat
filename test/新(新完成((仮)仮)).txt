...
const unsigned long interval = 100; // 0.1秒ごとに出力
...
struct SensorData {
  float hight_a, AcX, AcY, AcZ, GyX, GyY, GyZ, MagX, MagY, MagZ;
  float roll, pitch, yaw;
  String control_log;
} data;
...
void loop() {
  ...
  if (currentMillis - prevMillis >= interval) {
    ...
    SensorData d = displayInfo();
    display(d);
  }
}

// --------- 情報出力 ----------
SensorData displayInfo() {
  ...
  data.hight_a = ((P * T)/0.0065)-34.8;

  // ---- LIS3MDL ----
  LIS.read();
  data.MagX = LIS.m.x + 1034.8;
  data.MagY = LIS.m.y - 1656;
  data.MagZ = LIS.m.z;
...
void display(const SensorData &d) {
  Serial.print("高度: "); Serial.println(d.hight_a);

  Serial.print("加速度X[g]: "); Serial.println(d.AcX, 4);
  Serial.print("加速度Y[g]: "); Serial.println(d.AcY, 4);
  Serial.print("加速度Z[g]: "); Serial.println(d.AcZ, 4);

  Serial.print("ジャイロX[dps]: "); Serial.println(d.GyX, 4);
  Serial.print("ジャイロY[dps]: "); Serial.println(d.GyY, 4);
  Serial.print("ジャイロZ[dps]: "); Serial.println(d.GyZ, 4);

  Serial.print("磁気X: "); Serial.println(d.MagX);
  Serial.print("磁気Y: "); Serial.println(d.MagY);
  Serial.print("磁気Z: "); Serial.println(d.MagZ);

  Serial.print("Roll: "); Serial.println(d.roll);
  Serial.print("Pitch: "); Serial.println(d.pitch);
  Serial.print("Yaw: "); Serial.println(d.yaw);

  Serial.print("制御履歴: "); Serial.println(d.control_log);
  Serial.println("ーーーーーーーーーーーーーーーーーーーー");
  delay(1000);//1秒間隔
}
