...
- この関数は「非ブロッキング」方式->loop()内で関数を呼び出すたび、チェックし、新しいデータがあれば更新
...
### lookupangle
- 引数のl:目標地点までの距離
- 引数のthetadeg:目標方向と現在進行方向の角度の差
シータが小さい場合は15度から30度程度、舵を取る
大きい場合75度から140度程度、舵を取る
目標地点までの距離が
- hdop:GPS精度(小さいほど良い)
- Sats:衛星数
- amag:加速度の大きさ(1Gに近ければ安定)
- alt:高度
- vz:垂直速度
  
これらの状態が不安定の場合サーボを弱める
